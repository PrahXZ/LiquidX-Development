...
class VulcanSpeeds : SpeedMode("Vulcan") {
    ...
    private val modeValue = ListValue("Vulcan-Mode", arrayOf("Hop", "Hop2", "MiniHop", "Ground", "YPort", "YPort2"), "Hop")

    val boostDelayValue = IntegerValue("Boost-Delay", 8, 2, 15).displayable { modeValue.equals("Ground") }
    val boostSpeedValue = BoolValue("Ground-Boost", true).displayable { modeValue.equals("Ground") }
        ...
        when (modeValue.get()) {
            ...
            "MiniHop" -> {
                if (MovementUtils.isMoving()) {
                    if (mc.thePlayer.onGround) {
                        ticks = 0
                        mc.thePlayer.motionY = 0.24999998688697815f.toDouble()
                        MovementUtils.strafe(0.410F)
                        MovementUtils.move(0.052F)
                    } else {
                        ticks++
                        if (ticks == 7)
                            mc.thePlayer.motionY = -0.00000000001f.toDouble()
                        if (ticks == 4)
                            MovementUtils.move(0.01250F)
                        MovementUtils.strafe()
                    }
                }
            }
